...
public class NaturalNumber  {
	...
	public NaturalNumber plus( NaturalNumber  second) throws Exception{
		...
		//  ---------  BEGIN SOLUTION (plus)  ----------
		//
		int sizeClone = firstClone.coefficients.size();
		
		int[] colSum = new int[sizeClone+1];
        
		//update carry
		int carry = 0;
        for (int i=0; i<sizeClone ;i++) {    
        	colSum[i] = (firstClone.coefficients.get(i)+secondClone.coefficients.get(i)+carry) % getBase();
        	carry = (firstClone.coefficients.get(i)+secondClone.coefficients.get(i)+carry) /getBase();
        	sum.coefficients.add(i, colSum[i]);   
        } 
        //add digit if last carry is not 0
        if(carry!=0){
			sum.coefficients.add(sizeClone, carry);
        }
	...
	public NaturalNumber times( NaturalNumber multiplicand) throws Exception{
		...
		//I added zeros at each temporary product in the helper method
		
		for(int i=0; i<multiplicand.coefficients.size(); i++){
			product = product.plus(this.timesSingleDigit(i, multiplicand));
		}
	...
	public NaturalNumber timesSingleDigit(int j, NaturalNumber multiplicand) {
		
		NaturalNumber timesSingleDigit = new NaturalNumber(this.base);
		int carry = 0;
		
		//update carry during multiplication of single digits 
		for(int i=0;i<this.coefficients.size();i++){			
			timesSingleDigit.coefficients.add(i,(this.coefficients.get(i)*multiplicand.coefficients.get(j)+carry)%getBase()); 
			carry = (this.coefficients.get(i)*multiplicand.coefficients.get(j)+carry)/getBase();
		}
		if(carry!=0){
			timesSingleDigit.coefficients.add(this.coefficients.size(),carry);
        }
		//insert zeros at the end
		for(int m=0; m<j; m++){
			timesSingleDigit.coefficients.addFirst(0);
		} 
	...
	public NaturalNumber  minus(NaturalNumber second) throws Exception{
		...
		NaturalNumber  secondClo = new NaturalNumber(this.base);
		//add digits so that the two NaturalNumber have same size
		//create clone so that input values are not changed through calculation
		for (int i=0; i<second.coefficients.size() ;i++) {   
			secondClo.coefficients.add(i,second.coefficients.get(i));
		}
		if(this.coefficients.size() != second.coefficients.size()){
			for(int i=second.coefficients.size();i<this.coefficients.size();i++){
				secondClo.coefficients.addLast(0);
			}
		}		
		
		int borrow = 0;
		//borrow will be updated in the loop 
		//for each operation, check whether borrow is 0; if not, add base when subtracting
		for (int i=0; i<first.coefficients.size() ;i++) {   		
			if((first.coefficients.get(i) - secondClo.coefficients.get(i) - borrow) >= 0){
				difference.coefficients.add(i, (first.coefficients.get(i) - secondClo.coefficients.get(i) - borrow)%this.base); 
				borrow = 0;
			}
			if((first.coefficients.get(i) - secondClo.coefficients.get(i) - borrow) < 0){
				difference.coefficients.add(i, (first.coefficients.get(i) + this.base - secondClo.coefficients.get(i) - borrow)%this.base); //
				borrow = 1;
			}			
        }	
		//discrete the two cases and update borrow for each operation
	...
	public NaturalNumber divide( NaturalNumber  divisor ) throws Exception{
		...
		int div = divisor.coefficients.size();
		NaturalNumber toFindFirst = new NaturalNumber(this.base);
		NaturalNumber storeFirst = new NaturalNumber(this.base);

		for(int i=this.coefficients.size()-1;i>this.coefficients.size()-1-div;i--){
			toFindFirst.coefficients.addFirst(this.coefficients.get(i));
		}
		
		//test the same digits in base as divisor
		if (divisor.compareTo(toFindFirst) == -1||divisor.compareTo(toFindFirst) == 0){  
			storeFirst = toFindFirst.clone();
		}
		//otherwise add one more digit from the base
		if(divisor.compareTo(toFindFirst) == 1){
			storeFirst = toFindFirst.clone();
			storeFirst.coefficients.addFirst(this.coefficients.get(this.coefficients.size()-divisor.coefficients.size()-1));
		}			
		//position index for the leftmost digit in quotient
		int firstDigit = this.coefficients.size() - storeFirst.coefficients.size();	 
		//System.out.println("hi3:"+firstDigit); //check:correct!
		
	
		//use firstDigit to tell how many repetitions there would be
		for(int j=0;j<firstDigit+1;j++){  
			
			//the quotient would only take integers smaller than base
			//multiply base with divisor until we find the right digit
			//the tricky case is when there is 9 in the quotient
			
			for(int i=0;i<this.base+1; ){ //consider the case digit==base-1
				NaturalNumber tmpIndex = new NaturalNumber(this.base);
				tmpIndex.coefficients.addFirst(0);
				if(i<10){
					tmpIndex.coefficients.set(0,i);
				}
				if(i==10){
					tmpIndex.coefficients.addLast(1);
				}
				
				NaturalNumber tmpProduct = new NaturalNumber(this.base);
				tmpProduct = tmpIndex.times(divisor);	
				
				//for temporary Product, we want to add digits for it to subtract from "remainder"
				for(int k=firstDigit;k>j;k--){ 
					tmpProduct.coefficients.addFirst(0);
				}
				//System.out.println("print1"+tmpProduct); //check: correct
				
				//cases when tmpProduct is smaller than remainder,
				//keep incrementing the digit at quotient until tmpProduct is greater than remainder
				if(tmpProduct.compareTo(remainder) == -1 || tmpProduct.compareTo(remainder) == 0){
					i++;
				}else{
					//extract the prior digit that works
					
					NaturalNumber forHelpOne = new NaturalNumber(this.base);
					forHelpOne.coefficients.addFirst(1);
					quotient.coefficients.addFirst((tmpIndex.minus(forHelpOne)).coefficients.get(0));
					
					//System.out.println("hey"+forHelpOne); //check: correct
					//System.out.println("hey1"+tmpIndex.minus(forHelpOne));
					
					NaturalNumber forHelpTwo = new NaturalNumber(this.base);					
					forHelpTwo = (divisor.times(tmpIndex.minus(forHelpOne))).clone();
					
					//System.out.println("hey2"+forHelpTwo);
					
					for(int k=firstDigit;k>j;k--){ 
						forHelpTwo.coefficients.addFirst(0);
					}
					//System.out.println("hey3"+forHelpTwo);
					
					if(remainder.compareTo(divisor) != -1){  
						remainder = remainder.minus(forHelpTwo);
					}
					
					//after storing remainder, we exit this loop and keep looking for the next digit in quotient
					i=this.base+1;
				}												
			}			
		}
		
		//System.out.println("hi2"+remainder);
